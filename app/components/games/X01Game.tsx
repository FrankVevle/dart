...
export function X01Game({ onExit, duo = false }: { onExit: () => void; duo?: boolean }) {
  ...
  const [sentBackNames, setSentBackNames] = useState<string[]>([]);
  ...
  function throwAt(segment: number, dartMultiplier: 1 | 2 | 3, isMiss = false) {
    ...
    setThrowSeq(s => s + 1);
    const sentBackIds = result.sentBackPlayerIds ?? [];
    setSentBackNames(
      sentBackIds.map(id => engine.players.find(p => p.id === id)?.name).filter((n): n is string => !!n)
    );
        ...
        <div className="card">
          ...
          <div className="checkbox-row">
            <input
              type="checkbox"
              id="sameScorePenalty"
              checked={sameScorePenalty}
              onChange={e => setSameScorePenalty(e.target.checked)}
            />
            <label htmlFor="sameScorePenalty">
              Duo-regel: havner du på samme poengsum som en motstander, sendes de tilbake til start
            </label>
          </div>
  ...
  return (
    <main className="page">
      ...
      {sentBackNames.length > 0 && (
        <div key={`sentback-${throwSeq}`} className="status-banner bust bust-anim">
          🔻 {sentBackNames.join(' og ')} traff samme poengsum og sendes tilbake til start!
        </div>
      )}
      {lastResult?.status === 'valid' &&
        !lastThrowWasMiss &&
        sentBackNames.length === 0 &&
        engine.currentTurnDarts.length === 0 && <div className="status-banner valid">Tur registrert.</div>}
